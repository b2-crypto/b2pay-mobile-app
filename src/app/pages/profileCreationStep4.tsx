    ...
    <View style={styles.parent}>
      <ScrollView style={styles.container}>
        <View style={styles.wrapper}>
          <Text style={styles.title}>{t?.pages.profileCreationStep4.title}</Text>
          <Text style={styles.subtitle}>{t?.pages.profileCreationStep4.subtitle}</Text>
          <Text style={styles.description}>{t?.pages.profileCreationStep4.description}</Text>
          <View style={[styles.passwordWrapper, { marginBottom: !isPasswordFocused ? 24 : 0 }]}>
            <Input
              label={t?.pages.profileCreationStep4['password-label']}
              ...
              {numberOfValidRules === 5 && (
                <View style={styles.securePasswordWrapper}>
                  ...
                  <Text style={[styles.securePasswordText, styles.grate]}>
                    {t?.pages.profileCreationStep4['password-secure-3']}
                  </Text>
                </View>
              )}
              {numberOfValidRules > 2 && numberOfValidRules < 5 && (
                <View style={styles.securePasswordWrapper}>
                  ...
                  <Text style={[styles.securePasswordText, styles.medium]}>
                    {t?.pages.profileCreationStep4['password-secure-2']}
                  </Text>
                </View>
              )}
              {numberOfValidRules > 0 && numberOfValidRules <= 2 && (
                <View style={styles.securePasswordWrapper}>
                  ...
                  <Text style={[styles.securePasswordText, styles.weak]}>
                    {t?.pages.profileCreationStep4['password-secure-1']}
                  </Text>
                </View>
              )}
            </>
          ...
          {isPasswordFocused && (
            <View style={styles.rules}>
              <View style={styles.rule}>
                ...
                <Text style={styles.ruleText}>{t?.pages.profileCreationStep4['password-rule-1']}</Text>
              </View>
              <View style={styles.rule}>
                <View style={styles.iconWrapper}>
                  {passwordRules.uppercase && <Icon name="checked" width={16} height={16} />}
                </View>
                <Text style={styles.ruleText}>{t?.pages.profileCreationStep4['password-rule-2']}</Text>
              </View>
              <View style={styles.rule}>
                <View style={styles.iconWrapper}>
                  {passwordRules.lowercase && <Icon name="checked" width={16} height={16} />}
                </View>
                <Text style={styles.ruleText}>{t?.pages.profileCreationStep4['password-rule-3']}</Text>
              </View>
              <View style={styles.rule}>
                <View style={styles.iconWrapper}>
                  {passwordRules.number && <Icon name="checked" width={16} height={16} />}
                </View>
                <Text style={styles.ruleText}>{t?.pages.profileCreationStep4['password-rule-4']}</Text>
              </View>
              <View style={styles.rule}>
                <View style={styles.iconWrapper}>
                  {passwordRules.special && <Icon name="checked" width={16} height={16} />}
                </View>
                <Text style={styles.ruleText}>{t?.pages.profileCreationStep4['password-rule-5']}</Text>
              </View>
            </View>
          )}

          <Input
            label={t?.pages.profileCreationStep4['confirm-password-label']}
          ...
          {password && confirmPassword && !matchPassword && (
            <View style={styles.securePasswordWrapper}>
              <Icon name={'infoDanger'} width={24} height={24} />
              <Text style={[styles.securePasswordText, styles.weak]}>
                {t?.pages.profileCreationStep4['password-no-match']}
              </Text>
            </View>
          )}
        </View>
